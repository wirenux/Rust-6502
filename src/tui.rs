...
fn render_footer(frame: &mut Frame, area: Rect, state: &TuiState) {
    let spans = if state.show_settings {
        ...
        vec![
            ...
            Span::raw(" Reset  "),
            Span::styled(" I ", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
            Span::raw(" IRQ  "),
...
fn render_home(frame: &mut Frame, state: &mut TuiState) {
    ...
    let items: Vec<ListItem> = state.available_files.iter().map(|item| {
        ...
        } else if item.contains("_demo") || item.contains("wozmon") {
                            ...
                            match key.code {
                                ...
                                KeyCode::Up => {
                                    if state.home_focus == HomeFocus::FileList && !state.available_files.is_empty() {
                                        ...
                                        } else if file_name.contains("wozmon") && file_name.ends_with(".bin") {
                                            state.start_addr_input = "FE80".to_string();
                                            state.force_address = true;
